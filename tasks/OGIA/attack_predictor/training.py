...
class TrainingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.max_epochs <= 0 or self.patience <= 0 or self.batch_size <= 0:
            raise ValueError(
                "epochs, patience and batch_size must be positive."
            )
...
def _loss(
    ...
    *,
    ...
    predicted_fraction = (
        output["multiplier"] - config.min_multiplier
    ) / multiplier_range
    target_fraction = (
        target_multiplier - config.min_multiplier
    ) / multiplier_range
    multiplier_loss = F.smooth_l1_loss(
        predicted_fraction,
        target_fraction,
    )
    ...
    total = (
        composition_loss
        + config.multiplier_loss_weight * multiplier_loss
    )
...
def _metrics(
    ...
    composition_l1 = float(
        np.mean(np.sum(np.abs(delta), axis=1))
    )

    numerator = np.sum(
        target_composition * predicted_composition,
        axis=1,
    )
    denominator = (
...
def _train_one_model(
    ...
    *,
    ...
    # Start both heads from the empirical training prior. With the first M0
    # datasets being intentionally small, this avoids wasting many epochs
    # moving from arbitrary outputs to the observed target range.
    with torch.no_grad():
        mean_composition = np.mean(train_composition, axis=0)
        composition_logits = np.log(
            np.clip(mean_composition, 1e-6, None)
        )
        ...
        mean_fraction = (
            float(np.mean(train_multiplier))
            - model_config.min_multiplier
        ) / multiplier_range
        mean_fraction = float(
            np.clip(mean_fraction, 1e-4, 1.0 - 1e-4)
        )
        multiplier_bias = math.log(
            mean_fraction / (1.0 - mean_fraction)
        )
    ...
    for epoch in range(1, epochs_to_run + 1):
        ...
        with torch.inference_mode():
            output = model(
                torch.from_numpy(validation_features).to(device)
            )
    ...
    *,
    ...
    if np.any(target_multiplier < config.min_multiplier) or np.any(
        ...
        raise ValueError(
            ...
            f"model=[{config.min_multiplier:.4f}, "
            f"{config.max_multiplier:.4f}]"
        )

    resolved_device = _resolve_device(device)
    folds = _make_folds(len(rows), config.folds, config.seed)
    ...
    for fold_index, validation_indices in enumerate(folds, start=1):
        ...
        if len(train_indices) == 0:
            raise ValueError(
                "Cross-validation produced an empty training fold."
            )
        ...
        if progress:
            progress(
                ...
                f"epoch={best_epoch} "
                f"composition_l1="
                f"{fold_metrics['composition_l1_distance']:.4f} "
                f"multiplier_mae="
                f"{fold_metrics['multiplier_mae']:.4f}"
            )
        ...
        oof_rows.append(
            {
                ...
                "target": {
                    "ship_weights": {
                        ship: float(
                            target_composition[index, ship_index]
                        )
                        for ship_index, ship in enumerate(ATTACK_SHIPS)
                    },
                    "points_multiplier": float(
                        target_multiplier[index]
                    ),
                },
                "prediction": {
                    "ship_weights": {
                        ship: float(
                            oof_composition[index, ship_index]
                        )
                        for ship_index, ship in enumerate(ATTACK_SHIPS)
                    },
                    "points_multiplier": float(
                        oof_multiplier[index]
                    ),
                },
            }
        )
